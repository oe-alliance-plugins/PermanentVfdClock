        ...
        if not inStandby and config.plugins.PermanentVfdClock.enabled.value:
            ...
            try:
                with open(VFD_PATH, "w") as vfd:
                    if config.plugins.PermanentVfdClock.timeonly.value:
                        vfd.write("   %2d:%02d" % (t.tm_hour, t.tm_min))
                    else:
                        vfd.write("%2d:%02d %d/%d" % (t.tm_hour, t.tm_min, t.tm_mday, t.tm_mon))
            except OSError:
                pass
...
def Plugins(**kwargs):
    ...
    if path.exists(VFD_PATH):
        return [
            ...
            PluginDescriptor(
                name=_("Permanent VFD Clock"),
                description=_("Show permanent clock in VFD"),
